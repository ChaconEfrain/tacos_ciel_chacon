...
const useEventForm = (initialError, initialData) => {
  ...
  const validateFormData = (data) => {
    ...
    //time validation
    if (data.time && data.time.length > 0) newError["time"] = "";
    if (data.time && data.time.length === 0)
      newError["time"] = "Una hora es requerida";
    //place validation
    if (data.place && data.place.length > 0) newError["place"] = "";
    if (data.place && data.place.length === 0)
      newError["place"] = "Un lugar es requerido";
    //people validation
